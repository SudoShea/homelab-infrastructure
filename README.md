...
├── .github/
│   └── workflows/
│       └── lint.yml                    # Ansible-lint CI workflow
├── docs/
...
│   │   ├── files/                        # Dashboard JSON definitions & provisioning configs
│   │   ├── tasks/main.yml                # Quadlet deployment & config task definitions
│   │   └── templates/                    # grafana.container.j2, loki.container.j2 & loki-config.yml.j2
│   ├── logging_shipper/                  # Vector log collection agent service
│   │   ├── handlers/main.yml             # Service restart handlers
│   │   ├── tasks/main.yml                # Vector Quadlet deployment task definitions
│   │   └── templates/                    # vector.container.j2 & vector.toml.j2
...
│   └── run.sh                            # Deployment wrapper script (check/run/maintenance modes)
...
## ⚙️ Quadlet & System Maintenance Management
Once deployed, container lifecycles are managed natively by `systemd` user services and timers on each node:
```bash
# Check status of the full Quadlet stack
systemctl --user status pihole.service unbound.service loki.service vector.service grafana.service

# Restart a Quadlet service
systemctl --user restart vector.service

# View live service logs via journalctl
journalctl --user -u vector.service -f

# Check automated weekly maintenance timer status
systemctl --user status homelab-maintenance.timer

# View live maintenance logs
journalctl --user -u homelab-maintenance.service -f
```
---
